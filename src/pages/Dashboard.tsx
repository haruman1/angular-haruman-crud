...
export default function Dashboard() {
  ...
  const totalValue = stocks.reduce((sum, s) => sum + s.jumlah_produk * s.harga_produk, 0);
    ...
    <Layout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Ringkasan data stok dan pergerakan</p>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-4">
                <div>
                  <div className="text-lg font-bold text-green-600">+{totalIn}</div>
                  <p className="text-xs text-muted-foreground">masuk</p>
                </div>
                <div>
                  <div className="text-lg font-bold text-red-600">-{totalOut}</div>
                ...
                {stocks.slice(0, 5).map((stock) => (
                  <div key={stock.id} className="flex items-center justify-between border-b pb-2">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Stok: {stock.jumlah_produk}</p>
                    </div>
                    <p className="font-semibold">{formatCurrency(stock.harga_produk)}</p>
                  </div>
                ))}
              </div>
            )}
